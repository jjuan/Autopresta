...
/*
 * Rewriter by JJuan: Add insertion to javaSessionId in table and validate for user in session throw excetion .
...
class GormTokenStorageService implements TokenStorageService {
    ...
    void storeToken(String tokenValue, UserDetails principal) {
        ...
        def session = RequestContextHolder.currentRequestAttributes().getSessionId()
        def conf = SpringSecurityUtils.securityConfig
        ...
        if(!session)
            throw new IllegalArgumentException("The session is null in authentication storage service")
        ...
        def userNameToken = findExistingTokenByUser(principal.username)
        if(userNameToken){
            throw new SessionAuthenticationException("El usuario ${principal.username} se encuentra en sesion activa")
        }
        dc.withTransaction { status ->
            def newTokenObject = dc.newInstance((tokenValuePropertyName): tokenValue, (usernamePropertyName): principal.username,sessionId: session)
            newTokenObject.save()
        }
    }
    ...
    private findExistingTokenByUser(String userName) {
        log.debug "Searching in GORM for UserDetails of username ${userName}"
        def conf = SpringSecurityUtils.securityConfig
        String tokenClassName = conf.rest.token.storage.gorm.tokenDomainClassName
        String usernamePropertyName = conf.rest.token.storage.gorm.usernamePropertyName
        def dc = grailsApplication.getClassForName(tokenClassName)
