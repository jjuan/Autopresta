...
@Component({
  ...
  templateUrl: './contrato-persona-fisica.component.html',
  styleUrls: ['./contrato-persona-fisica.component.sass']
})
export class ContratoPersonaFisicaComponent implements OnInit {
  public _datos = {
  ...
  form(data?) {
    this.formulario = this.genericRestService.buildForm({
      ...
      montoRequerido: [data ? data.montoRequerido : '', [Validators.min(20000), (control: AbstractControl) => Validators.max(Number((this.montoMaximoAutorizado).toFixed(2)))(control)]],
