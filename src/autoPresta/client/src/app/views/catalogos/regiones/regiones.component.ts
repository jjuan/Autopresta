...
import {Regiones} from "../../../core/models/data.interface";
...
import {FormRegionesComponent} from "./form-regiones/form-regiones.component";

@Component({
  selector: 'app-regiones',
  templateUrl: './regiones.component.html',
  styleUrls: ['./regiones.component.sass']
})
export class RegionesComponent implements OnInit {
  datos = {modulo: 'Catalogos', componente: 'Regiones', icono: 'fas fa-folder-open', titulo: 'Regiones', controlador: 'Regiones'}
  displayedColumns = [
    'select',
    // 'id',
    'clave',
  ...
  selection = new SelectionModel<Regiones>(true, []);
  ...
  save(row?: Regiones) {
    if (row){
      this.restService.edit<Regiones>(row.id, this.datos.controlador).subscribe(result => {
        const dialogRef = this.dialog.open(FormRegionesComponent, {
          data: { title: row.clave , disableClose: true, data: result, action: 'Editar' }, height: 'auto', width: '40%'
        });
      ...
      this.restService.create<Regiones>(this.datos.controlador).subscribe(result => {
        const dialogRef = this.dialog.open(FormRegionesComponent, {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.refreshTable();
      this.selection = new SelectionModel<Regiones>(true, []);
    });
  ...
  delete(row: Regiones) {
    const html ='<div class="align-left mt-3">'+
      '<h5>Detalles del ' + this.datos.titulo.toLowerCase() + ': ' + row.clave + '</h5>'+
      '<p><span class="font-weight-bold">Clave: </span>' + row.clave + '</p>' +
      '<p><span class="font-weight-bold">Descripcion: </span>' + row.descripcion + '</p>' +
      '<p><span class="font-weight-bold">Variacion: </span>' + row.variacion + '</p>' +
      '</div>';
    Swal.fire({
...
export class registros extends DataSource<Regiones> {
  ...
  filteredData: Regiones[] = [];
  renderedData: Regiones[] = [];
  ...
  connect(): Observable<Regiones[]> {
    ...
    return merge(...displayDataChanges).pipe(map(() => {
        this.filteredData = this.ds.data.slice().filter((campo: Regiones) => {
          const searchStr = (
            campo?.clave +
            campo?.descripcion
          ).toLowerCase();
  ...
  sortData(data: Regiones[]): Regiones[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        ...
        case 'descripcion':
          ...
          break;
        case 'variacion':
          [propertyA, propertyB] = [a.variacion, b.variacion]
          break;
      }
