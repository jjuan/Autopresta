...
import {FechaInhabil} from "../../../core/models/data.interface";
...
import {FormFechaInhabilComponent} from "./form-fecha-inhabil/form-fecha-inhabil.component";

@Component({
  selector: 'app-fecha-inhabil',
  templateUrl: './fecha-inhabil.component.html',
  styleUrls: ['./fecha-inhabil.component.sass']
})
export class FechaInhabilComponent implements OnInit {
  datos = {modulo: 'Catalogos', componente: 'FechaInhabil', icono: 'fas fa-folder-open', titulo: 'FechaInhabil', controlador: 'FechaInhabil'}
  displayedColumns = [
    'select',
    // 'id',
    'fecha',
  ...
  selection = new SelectionModel<FechaInhabil>(true, []);
  ...
  save(row?: FechaInhabil) {
    if (row){
      this.restService.edit<FechaInhabil>(row.id, this.datos.controlador).subscribe(result => {
        const dialogRef = this.dialog.open(FormFechaInhabilComponent, {
      ...
      this.restService.create<FechaInhabil>(this.datos.controlador).subscribe(result => {
        const dialogRef = this.dialog.open(FormFechaInhabilComponent, {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.refreshTable();
      this.selection = new SelectionModel<FechaInhabil>(true, []);
    });
    this.dialogService.snack( 'danger', totalSelect + ' registros eliminados' );
  }

  delete(row: FechaInhabil) {
    const html ='<div class="align-left mt-3">'+
      '<h5>Detalles del ' + this.datos.titulo.toLowerCase() + ': ' + row.descripcion + '</h5>'+
      '<p><span class="font-weight-bold">Fecha: </span>' + row.fecha + '</p>' +
      '<p><span class="font-weight-bold">Descripcion: </span>' + row.descripcion + '</p>' +
      '<p><span class="font-weight-bold">Divisa: </span>' + row.divisa + '</p>' +
      '</div>';
    Swal.fire({
      titleText: this.datos.titulo,
      html: '¿Esta seguro de eliminar el registro: ' + row.descripcion + '?<br>' + html,
      icon: 'warning',
...
export class registros extends DataSource<FechaInhabil> {
  ...
  filteredData: FechaInhabil[] = [];
  renderedData: FechaInhabil[] = [];
  ...
  connect(): Observable<FechaInhabil[]> {
    ...
    return merge(...displayDataChanges).pipe(map(() => {
        this.filteredData = this.ds.data.slice().filter((campo: FechaInhabil) => {
          const searchStr = (
            campo?.fecha +
            campo?.descripcion
          ).toLowerCase();
  ...
  sortData(data: FechaInhabil[]): FechaInhabil[] {
