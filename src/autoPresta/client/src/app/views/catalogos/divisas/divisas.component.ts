...
import {FormDivisasComponent} from "./form-divisas/form-divisas.component";
...
@Component({
  ...
  styleUrls: ['./divisas.component.sass']
})
export class DivisasComponent implements OnInit {
  datos = {modulo: 'Catalogos', componente: 'Divisa', icono: 'fas fa-folder-open', titulo: 'Divisas', controlador: 'Divisas'}
  displayedColumns = [
    'select',
    // 'id',
    'clave',
  ...
  selection = new SelectionModel<Divisas>(true, []);
  ...
  save(row?: Divisas) {
    if (row){
      this.restService.edit<Divisas>(row.id, this.datos.controlador).subscribe(result => {
        const dialogRef = this.dialog.open(FormDivisasComponent, {
          data: { title: row.clave , disableClose: true, data: result, action: 'Editar' }, height: 'auto', width: '40%'
        });
        dialogRef.afterClosed().subscribe(result => {
          if (!result) { return; }
          this.restService.update<string>(row.clave, result, this.datos.controlador).subscribe(() => {
      ...
      this.restService.create<Divisas>( this.datos.controlador).subscribe(result => {
        const dialogRef = this.dialog.open(FormDivisasComponent, {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.refreshTable();
      this.selection = new SelectionModel<Divisas>(true, []);
    });
  ...
  delete(row: Divisas) {
    ...
    Swal.fire({
      ...
      html: '¿Esta seguro de eliminar el registro: ' + row.clave + '?<br>' + html,
      icon: 'warning',
...
export class registros extends DataSource<Divisas> {
  filterChange = new BehaviorSubject('');
  get filter(): string { return this.filterChange.value; }
  set filter(filter: string) { this.filterChange.next(filter); }
  filteredData: Divisas[] = [];
  renderedData: Divisas[] = [];
