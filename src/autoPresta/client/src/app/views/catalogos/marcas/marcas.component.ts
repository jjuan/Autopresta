...
import {Marcas} from "../../../core/models/data.interface";
...
export class MarcasComponent implements OnInit {
  ...
  save(row?: Marcas) {
    if (row){
      this.restService.edit<Marcas>(row.id, this.datos.controlador).subscribe(result => {
        const dialogRef = this.dialog.open(FormMarcasComponent, {
      ...
      this.restService.create<Marcas>( this.datos.controlador).subscribe(result => {
        const dialogRef = this.dialog.open(FormMarcasComponent, {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.refreshTable();
      this.selection = new SelectionModel<Marcas>(true, []);
    });
...
export class registros extends DataSource<Marcas> {
  ...
  connect(): Observable<Marcas[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this.ds.data.slice().filter((campo: Marcas) => {
          const searchStr = (
            campo.id +
            campo.nombre+
            campo.agencia
